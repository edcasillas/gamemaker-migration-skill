...
### Legacy save/highscore calls may not survive HTML5
...
Commit `365efb3` found that MIDI files were not supported in the target GMS/browser path and converted the main-menu music to OGG. Commit `4987748` completed background music conversion. The sound resource metadata changed extension/data from `.mid` to `.ogg`, and compression/streaming settings changed with it.
...
### Obsolete wait/sleep actions need behavioral validation

The compatibility report flagged obsolete sleep strings in player collisions. Earlier fixes replaced them with logging placeholders while the project moved toward non-blocking timing.

Reusable rule: do not emulate blocking sleep in HTML5. Preserve the visible delay with alarms, Step state, timed-action managers, or transition controllers, then replay the affected path.
